...
class FaceRWKV(nn.Module):
    def __init__(self, config):
        ...
        if self.resnet:
            ...
            self.num_patches = config.resolution[0] * \
                config.resolution[1]//(config.patch_size**2)
            config.ctx_len = self.num_patches  # reset the context length
        ...
        if self.block == "rwkv":
            self.blocks = nn.Sequential(
                *[Block(config, i) for i in range(self.n_layers)])
        elif self.block == "transformer":
            # use transformer blocks
            self.blocks = nn.TransformerEncoder(nn.TransformerEncoderLayer(
                d_model=self.embed_dim, nhead=config.n_head, dim_feedforward=self.n_ffn, batch_first=True), num_layers=self.n_layers)
...
class RWKVConfig:
    def __init__(self):
        ...
        # Scale for weight initialization in RWKV_TimeMix and RWKV_ChannelMix
        self.scale_init = 0
